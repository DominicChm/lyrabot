      ...
      .then(function(connection) {
        const dispatcher = connection
          ...
          .on('start', function() {
            ...
            if (message.guild.musicData.id == 0) {
              message.say(videoEmbed).then(sent => { // 'sent' is that message you just sent
              message.guild.musicData.id = sent.id;
              console.log(message.guild.musicData.id);
              });
            } else {
              message.channel.messages.fetch({around: message.guild.musicData.id, limit: 1})
              .then(msg => {
                  const fetchedMsg = msg.first();
                  fetchedMsg.edit(videoEmbed);
              });              
            }
          ...
          .on('error', function(e) {
            message.say(':x: Cannot play song!' + queue[0].title);
            console.error(e);
            if (queue.length > 1) {
              queue.shift();
              classThis.playSong(queue, message);
